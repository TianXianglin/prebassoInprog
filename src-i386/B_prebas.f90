...
!define varibles
 ...
 real (kind=8) :: H, D, B, Hc, Cw, Lc, N, Ntree, species,Ntot,dNtot
 real (kind=8) :: wf_treeKG, wf_STKG, sar_con, sar_ell, rc, ppow, sar,W_stem
 real (kind=8) ::  lproj, leff,laPer_sar, keff, slc
 real (kind=8) :: hb, check, A, B2,beta0, beta1,beta2, betas, betab
...
!  par_vr = par_vr / (1. + par_aETS * (ETS-ETS_ref)/ETS_ref)
 ...
 !calculate derived variables 
  ...
  hb = par_betab * Lc**par_x !crown radius - AM: exponent should change with species 
  Cw = 2 * hb  ! added here AM 15.5.2015
  ppow=1.6075 
...
if (year <= maxYearSite) then
   ...
   if(fAPARsite == 0 .and. yearX == 0) then
	Ainit = 6 + 2*3.5 - 0.005*(sum(modOut(year:(year+9),5,1,1))/10) + 2.25
	yearX = Ainit + year
...
!  If (checkThin(ij) > 0 .and. time==inttimes) Then 
